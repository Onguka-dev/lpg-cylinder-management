...
import { SellingPointReceiveActions } from "@/components/selling-point-receive-actions";
import { prisma } from "@/lib/prisma";
import { formatSellingPointLabel } from "@/lib/selling-point-distribution";

export default async function SellingPointDispatchDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-panel">
        <Link className="text-sm font-semibold text-brand-700 hover:underline" href="/inventory/selling-point-dispatches">Back to selling point dispatches</Link>
        <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-sm font-semibold text-brand-700">{formatSellingPointLabel(movement.status)}</p>
            <h1 className="mt-2 text-2xl font-semibold text-slate-950">{movement.reference}</h1>
            <p className="mt-2 text-sm text-slate-600">{movement.sourceLocation?.name ?? "Wandiege"} to {movement.destinationLocation?.name ?? "destination"}</p>
          </div>
      ...
      <SellingPointReceiveActions movementId={movement.id} defaultCodes={codes} status={movement.status} />

      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-panel">
        <h2 className="text-base font-semibold text-slate-950">Cylinder scan list</h2>
        <div className="mt-4 overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {movement.cylinders.map((line) => (
                <tr key={line.id}>
                  <td className="px-3 py-2 font-mono">{line.cylinder.barcode ?? "-"}</td>
                  <td className="px-3 py-2">{line.cylinder.serialNumber}</td>
                  <td className="px-3 py-2">{line.cylinder.cylinderSizeKg ?? "-"}kg</td>
                  <td className="px-3 py-2">{formatSellingPointLabel(line.cylinder.status)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-panel">
        ...
        <div className="mt-3 space-y-3">
          {movement.historyEntries.map((entry) => (
            <div className="rounded-lg border border-slate-100 p-3 text-sm" key={entry.id}>
              <p className="font-semibold text-slate-900">{entry.action}</p>
              <p className="mt-1 text-slate-600">{entry.details}</p>
              <p className="mt-1 text-xs text-slate-500">{entry.createdAt.toLocaleString("en-KE")}</p>
            </div>
          ))}
        </div>
      </section>
