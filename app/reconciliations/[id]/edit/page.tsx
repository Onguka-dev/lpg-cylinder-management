...
export default async function EditReconciliationPage({ params }: { params: { id: string } }) {
  ...
  const reconciliation = await prisma.dailyReconciliation.findUnique({ where: { id: params.id }, include: { countLines: true } });
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-panel">
        <ReconciliationForm reconciliation={reconciliation} users={users} skus={skus} currentUserId={session.user.id} currentRole={session.user.role} />
      </section>
    </div>
  );
}
